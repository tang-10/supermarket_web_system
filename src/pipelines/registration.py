...
class ProductRegistrationPipeline:
    ...
    def run(self, video_path: str, req: ProductRegisterRequest) -> bool:
        ...
        try:
            ...
            print(
                f"抽帧完成，原始帧数: {len(raw_frames)}, 去重后帧数: {len(dedup_frames)}"
            )
            ...
            if product_id is None:
                print("[注册失败] MySQL 写入阶段出错")
                return False

            try:
                # 存入 FAISS 向量库
                self.vector_db.insert(
                    np.stack(representative_features, axis=0), product_id
                )
            except Exception as e:
                print(f"[注册失败] 向量库写入出错: {e}")
                return False
        except Exception as e:
            print(f"[系统异常] 注册流程中断: {e}")
            return False
        return True
